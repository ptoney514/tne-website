    ...
    const dbData = {
      ...
      waiverMediaAcceptedAt:
        registration.waiver_media
          ? new Date(registration.waiver_accepted_at || new Date())
          : null,
      parentPolicyAccepted: !!registration.parent_policy,
      parentPolicyAcceptedAt:
        registration.parent_policy
          ? new Date(registration.waiver_accepted_at || new Date())
          : null,
      paymentStatus: mapPaymentStatus(registration.payment_status || registration.payment_plan_type),
...
/**
 * Map payment status / plan type to a valid DB enum value.
 * DB enum allows: 'pending', 'paid', 'partial', 'waived'
 */
function mapPaymentStatus(value) {
  switch (value) {
    // Valid DB values — pass through
    case 'pending':
    case 'paid':
    case 'partial':
    case 'waived':
      return value;
    // Hook-generated statuses → map to valid enum
    case 'pending_verification':
    case 'awaiting_approval':
    case 'pending_team_placement':
      return 'pending';
    case 'payment_plan_active':
      return 'partial';
    // Payment plan types (fallback)
    case 'full':
      return 'pending';
    case 'installment':
      return 'partial';
