...
export default function PaymentCommitmentStep() {
  ...
  const canProceed = (() => {
    ...
    // make_arrangements and full are always ready once selected
    return true;
  })();
  ...
  // Show inline payment info when a pay-now option is selected
  const showPaymentInfo = formData.paymentPlanType === 'full' || formData.paymentPlanType === 'installment';
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-neutral-600 mt-1">
          Select how you would like to handle the ${totalFee.toFixed(2)} registration fee
        </p>
      </div>
      ...
      <div className="space-y-4">
        ...
        {/* Inline Payment Info (shown for pay-now options) */}
        {showPaymentInfo && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-2">
                <p className="text-sm text-blue-800">
                  Complete your payment after registration using one of these methods.
                  Include your reference ID: <strong className="font-mono">{paymentReferenceId}</strong>
                </p>
                <div className="flex flex-wrap gap-4 text-xs text-blue-700">
                  <span><strong>PayPal:</strong> {PAYMENT_METHODS.paypal.displayUrl}</span>
                  <span><strong>Venmo:</strong> {PAYMENT_METHODS.venmo.handle}</span>
                  <span><strong>Cash App:</strong> {PAYMENT_METHODS.cashapp.handle}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        >
          <button
            type="button"
            onClick={() => handleSelectPaymentType('make_arrangements')}
            className="w-full text-left p-4"
          >
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-semibold text-neutral-900">Make Arrangements</h4>
                <p className="text-sm text-neutral-600">
                  Register now and we&apos;ll contact you to discuss payment options
                </p>
              </div>
            </div>
          </button>
            ...
            <div className="px-4 pb-4 space-y-3">
              <div className="p-3 rounded-xl bg-amber-100 border border-amber-200">
                <p className="text-sm text-amber-800">
                  Your player will be registered immediately. Our team will reach out
                  to discuss payment options that work for you.
                </p>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => updateField('specialRequestNotes', e.target.value)}
                  rows={2}
